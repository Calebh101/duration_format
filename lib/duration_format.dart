...
class DurationFormat {
  ...
  DurationFormat.clock({this.showSeconds = false, this.showMinutes = true, this.showHours = true, this.showDays = true, this.showMilliseconds = DurationFormatMillisecondsClockMode.hide, this.millisecondDigits = 2}) {
    mode = 1;
    if (showMilliseconds.runtimeType != DurationFormatMillisecondsClockMode) throw Exception("showMilliseconds must be of type \"DurationFormatMillisecondsClockMode\" in DurationFormat.clock().");
    
    List list = _generateList(showMilliseconds: showMilliseconds != DurationFormatMillisecondsClockMode.hide);
    if (millisecondDigits < 2 || millisecondDigits > 3) throw Exception("millisecondsDigits must be a valid integer between 2 and 3 (inclusive).");
    if (list.where((value) => value).length < 2) throw Exception("At least 2 time nodes need to be shown in the formatter.");
    if (_invalidSelection(list)) throw Exception("Invalid list of time nodes based on boolean values.");
  }
  ...
  DurationFormat.pretty({this.showSeconds = false, this.showHours = true, this.showMinutes = true, this.showDays = true, this.showMilliseconds = false, this.finalPhraseMode = DurationFormatFinalPhraseMode.commaAnd, this.millisecondDigits = 3}) {
    mode = 2;
...
String _format(Duration duration, DurationFormat formatter) {
  ...
  bool showDays = formatter.showDays && (times["days"] > 0 || formatter.showHours == false);
  bool showHours = formatter.showHours && (times["hours"] > 0 || formatter.showMinutes == false);
  ...
  if (showHours) values.add({"key": "hour", "value": "${times["hours"]}".padLeft(showDays && formatter.mode == 1 ? 2 : 0, "0")});
  if (showMinutes) values.add({"key": "minute", "value": "${times["minutes"]}".padLeft(showHours && formatter.mode == 1 ? 2 : 0, "0")});
  if (showSeconds) values.add({"key": "second", "value": "${times["seconds"]}".padLeft(showMinutes && formatter.mode == 1 ? 2 : 0, "0")});
  if (showMilliseconds) values.add({"key": "millisecond", "value": "${times["milliseconds"]}".padLeft(3, "0").substring(0, formatter.millisecondDigits)});

  if (values.length == 2 && formatter.finalPhraseMode == DurationFormatFinalPhraseMode.commaAnd) formatter.finalPhraseMode = DurationFormatFinalPhraseMode.and;
